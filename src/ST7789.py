# Downloaded from: https://github.com/XuBovey/MicroPython-ST7789/blob/master/ST7789.py
...
# Slightly modified by @PaulskPt: 
# Added the function set_offset(x, y). Needed to set the self._offset to 'bytearray([0,80])',
#  when using this driver with an Unexpected Maker Explorer Shield with an Unexpected Maker TinyPICO V2 (USB-C).
# Functions added by @PaulskPt: 
#  set_offset(x, y)
#  set_size(x, y)
#  get_rotation()
#
...
#Looking at display with pins on top.elf._offset = bytearray([0,80]) 
...
#TFTRotations = [(0x00, 240, 240, 0, 0), (0x60, 240, 240, 0, 0), (0xc0, 240, 240, 0, 80), (0xa0, 240, 240, 80, 0)]
...
ScreenSize = (240, 240) # was: (240, 320)
boardName = "UM_7789" # was: "SKIDS_7789"
...
class TFT(object) :
  ...
  def __init__( self, spi, aDC, aCS, aReset=None) :
    ...
    """
       Orignally: self._offset = bytearray([0,0]).
       This caused 40% of lower part of the display not being useable.
       Setting it to [0, 80] solved this problem on a Unexpected Maker TinyPICO Explorer Shield
       with an Unexpected Maker TinyPICO V2.
       Offset can now be set by calling the added function set_offset(x, y)
    """
  ...
  # Function added by @PaulskPt on 2022-08-22
  def set_offset(self, x, y):
      offset = bytearray([0,0]) # default
      if x >= 0 and x <= 320:
          if y >= 0 and y <= 320:
              offset = bytearray([x,y])
      self._offset = offset
...
#   @micropython.native
  def text( self, aPos, aString, aColor, aFont, aSize = 1, nowrap = False ) :
    ...
    if (type(aSize) == int) or (type(aSize) == float):
      ...
      wh = aSize
    """
    print("type(aFont)=", type(aFont))
    print("aFont[\"Width\"]=", end='')
    print(aFont["Width"])
    """
    px, py = aPos
...
#   @micropython.native
  def char( self, aPos, aChar, aColor, aFont, aSizes ) :
    ...
    startchar = aFont['Start']   # aFont['Start']
    endchar = aFont['End'] # aFont['End']
    ...
    if (startchar <= ci <= endchar):
      ...
      charA = aFont["Data"][ci:ci + fontw]  # aFont["Data"][ci:ci + fontw] 
      px = aPos[0]
